...
| `show_source`                    | boolean | Dynamic        | If `true`/`false`, explicitly toggles the visibility of the feed's source next to each item. By default, it is only displayed when using multiple `entities`.                                                             |
