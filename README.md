...
- **Keeps Playing**: Audio continues when you switch views or dashboards, with lock-screen and media-key controls. Only one episode plays at a time.
- **Play on a Speaker**: Set `audio_target` to send episodes to a `media_player` entity instead of the browser.
...
| `description_attribute`          | string  | _optional_     | An entry attribute to show as the item text instead of `summary`/`description`, for feeds that keep their text in a field of their own (e.g. `gera_description` on RP Online). Items without it fall back to the summary.                                                                              |
...
| `show_source`                    | boolean | Dynamic        | If `true`/`false`, explicitly toggles the visibility of the feed's source next to each item. When displayed, it dynamically extracts the item's internal `category` or `source` metadata before falling back to the parent entity's name. By default, it is only shown when using multiple `entities`. |
...
| `audio_target`                   | string  | _optional_     | A `media_player` entity to play episodes on (e.g. a Cast or Sonos speaker) instead of in the browser. Seeking is offered only if the player supports it.                                                                                                                                               |
